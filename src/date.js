      ...
      handles.listeners.push(ctx.listen('keydown', (/** @type { KeyboardEvent } */ ev) => {
        ...
        switch (ev.key) {
          ...
          case 'ArrowDown': {
            ...
            if (g.value === null) {
              if (g.type === 'a') g.value = 'AM';
              else g.value = 1;
            }

            if (g.type === 'a') syncAP(groups, g.value === 'AM' ? 'PM' : 'AM', g);
            else bumpValue(g, ev.key === 'ArrowDown');
...
function syncAP(groups, next, a, h) {
  a = a || groups.find(g => g.type === 'a');
  if (!a) return;
  h = h || groups.find(g => g.type.toUpperCase() === 'H');
  if (!h) return;
...
function readInput(groups, node, mask) {
  ...
  const ap = groups.find(g => g.type === 'a');
...
function applyValues(groups, send, focused, defaultDate, defaultTime) {
  ...
  if (!groups.find(p => p.type === 'm' || p.type === 's' || p.type === 'H' || p.type === 'h' || p.type === 'S' || p.type === 'a')) {
  ...
  if (parts[0] !== null && parts[1] !== null && parts[2] !== null) {
    ...
    ['m', 's', 'h', 'H', 'S'].forEach(p => {
      const g = groups.find(g => g.type === p);
      if (g && g.value === null) {
  ...
  if (groups.find(g => g.value === null && g.type !== 'a') || parts[0] === 0 || parts[1] > 11 || parts[1] < 0 || parts[2] === 0) {
    groups.value = null;
  } else {
    parts.unshift(null);
...
function displayForGroup(group) {
  if (group.value === null || group.value === undefined) {
    if (group.type === 'a') {
      const h = group.groups.find(g => g.type.toUpperCase() === 'H');
      if (!h || h.value === null || h.value === undefined) return padl('', 2, blankChar);
    } else {
  ...
  switch (group.type) {
    ...
    case 'a':
      const h = group.groups.find(g => g.type === 'h' || g.type === 'H');
