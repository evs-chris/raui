      ...
      function plugin$1(opts) {
        ...
        return function(ref) {
          ...
          proto.toast = function toast(message, options) {
            ...
            opts.close = function () { return closeToast(opts); };
            toasts.push(opts);
            if (!active) { showToast(); }
            return new Handle(holder, opts);
          };

          function closeToast(which) {
            if (active !== which) {
      ...
      var Host = (function (Base) {
        ...
        Host.prototype.raise = function raise (window, opts) {
          ...
          if (top && top !== last && this.get('currentMax') && this.rendered) {
            ...
            if (transition !== false && (!leaving || !leaving.dialog || (leaving.blocking && leaving.blocking !== top))) { wnd.transition(transition || 'window', wnd.find('div'), { intro: true }); }
          }
        };
