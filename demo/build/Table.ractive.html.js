System.register(['ractive', './chunk7.js', './chunk10.js', './chunk2.js', './chunk14.js', './chunk18.js'], function (exports, module) {
  'use strict';
    ...
    execute: function () {
      ...
      function scrollother(node, find) {
        ...
        var scroll = function (ev) {
          if (scrollother.lock && scrollother.lock !== node) { return; }

          if (scrollother.tm) { clearTimeout(scrollother.tm); }
          scrollother.lock = node;
          this$1.find(find).scrollLeft = ev.target.scrollLeft;
          scrollother.tm = setTimeout(function () {
            scrollother.tm = null;
            scrollother.lock = false;
          }, 100);
        };
      ...
      Ractive$1.extendWith(Table, {
        ...
        cssId: 'rtable',
        noCssTransform: true,
        attributes: ['auto-titles', 'paginate', 'items', 'filter', 'data-handler', 'sort-handler', 'sort', 'helpers', 'fixed', 'allow-select', 'allow-select-all', 'border', 'fixed-header', 'valign', 'no-header', 'empty', 'shuffle', 'no-wrap', 'parameters'],
        components: {
          table: false,
          Table: false
        },
          ...
          tpl.forEach(function (e) {
            if (e.e === 'column' || e.e === 'col') {
              ...
              if (col.label) {
                ...
                col.title = attrs.find(function (a) { return a.n === 'label'; });
                if (!col.title) { col.title = attrs.find(function (a) { return a.n === 'header'; }); }
                if (col.title && col.title.f) { col.title = map(col.title); }
              } else {
