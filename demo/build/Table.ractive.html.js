System.register(['ractive', './chunk7.js', './chunk8.js', './chunk2.js', './chunk12.js', './chunk14.js'], function (exports, module) {
  'use strict';
  ...
  return {
    ...
    execute: function () {
      ...
      var Table = (function (Ractive) {
        ...
        var prototypeAccessors = { selections: { configurable: true } };
        ...
        prototypeAccessors.selections.get = function () {
      ...
      function columnGetter(table, col) {
        ...
        if (!getters[k]) {
          if (isString(v) && ~v.indexOf('*')) {
            var parts = Ractive$1.splitKeypath(v);
            getters[k] = function (obj) {
              var res = applyPathReduce(obj, parts);
              return res.v || res.l;
            };
          } else {
      ...
      function scrollheader(node) {
        var this$1 = this;

        var scroll = function (ev) {
          requestAnimationFrame(function () { return this$1.find('.rtable-header-wrapper').scrollLeft = ev.target.scrollLeft; });
        };
      ...
      Ractive$1.extendWith(Table, {
        ...
        cssId: 'rtable',
        noCssTransform: true,
        attributes: ['paginate', 'items', 'filter', 'sort', 'helpers', 'fixed', 'allow-select', 'allow-select-all', 'border', 'fixed-header', 'valign', 'no-header', 'empty', 'shuffle', 'no-wrap'],
        components: {
          table: false,
          Table: false
        },
        syncComputedChildren: true,
        decorators: { grid: grid, sized: sized, scrollheader: scrollheader },
        events: {},
          ...
          rows: {
            get: function get() {
              ...
              if ((filter instanceof RegExp || isString(filter)) && filter) {
                ...
                var flts = buildFilter(this, filter, columns, fields);
                var recache = {};
                try {
                  list = list.filter(function (l) {
      ...
      function applyPathReduce(src, path) {
        if (!path.length) { return { v: src }; }
        if (path.length && typeof src !== 'object') { return { n: 1 }; }
        var key = path[0];
        if (key === '*' && Array.isArray(src)) {
          var next = path.slice(1);
          return { l: src.reduce(function (a, c) {
            var v = applyPathReduce(c, next);
            if (isObject(v)) {
              if (v.v) { a.push(v.v); }
              else if (v.l) { a.push.apply(a, v.l); }
              else if (!v.n) { a.push(v); }
            } else { a.push(v); }
            return a;
          }, []) };
        } else {
          return applyPathReduce(src[key], path.slice(1));
        }
      }
      ...
      function buildFilter(grid, filter, columns, fields) {
        return filter.map(function (f) {
          ...
          if (f.id) {
            ...
            if (!field) {
              ...
              if (field) {
                flt.get = fieldGetter(grid, field);
                flt.type = field.type || 'string';
              }
            }
          } else {
          ...
          return flt;
        }).filter(function (f) { return f.op !== 'skip'; });
      }
      ...
      function construct() {
        ...
        var expandEl, groupEl, wrapperEl, rowEl, topEl, bottomEl, mappings;
        ...
        function grabColumns(tpl) {
          ...
          tpl.forEach(function (e) {
            if (e.e === 'column' || e.e === 'col') {
              ...
              if (col.fixed && isObject(col.fixed.f)) { col.fixed = map(col.fixed); }
        ...
        var header = [{ t: 7, e: 'div', m: [
          ...
          var div = { t: 7, e: 'div', f: [{ t: 7, e: 'div', f: c.label }], m: [{ t: 4, n: 53, r: ("~/columns." + cidx), f: [{ t: 70, n: ['click'], f: { r: ['@this', '.index', '@event'], s: sortKey } }] }, { t: 13, n: 'title', f: c.label }, { t: 16, r: ("~/columns." + cidx + ".attrsHP") }] };
          ...
          if (c.fixed) {
            div.m.push({ t: 13, n: 'class-rtable-fixed-column', f: [{ t: 2, r: ("~/columns." + cidx + ".fixed") }] });
            div.m.push({ t: 4, n: 50, x: { r: ["~/noWrap", ("~/columns." + cidx + ".fixed")], s: "_0&&typeof _1==='string'" }, f: [{ t: 13, n: 'style-left', f: [{ t: 2, r: ("~/columns." + cidx + ".fixed")}] }] });
          }
          var res = div;
        ...
        var row = [{ t: 7, e: 'div', m: [
          ...
          var attrs = c.attrs;
          if (c.fixed) {
            attrs.push({ t: 13, n: 'class-rtable-fixed-column', f: [{ t: 2, r: ("~/columns." + cidx + ".fixed") }] });
            attrs.push({ t: 4, n: 50, x: { r: ["~/noWrap", ("~/columns." + cidx + ".fixed")], s: "_0&&typeof _1==='string'" }, f: [{ t: 13, n: 'style-left', f: [{ t: 2, r: ("~/columns." + cidx + ".fixed")}] }] });
          }
        ...
        if (rowEl && rowEl.m && rowEl.m.length) {
          ...
          row[0].f[0].f[0].m.push({ t: 16, r: '~/rowAttrs', c: { r: '.' } });
        }
        ...
        if (rowEl && rowEl.f && rowEl.f.length) {
          row[0].f[0].f.push({ t: 7, e: 'div', m: [{ t: 13, n: 'class-rtable-row-extra' }, { t: 4, n: 50, r: '~/noWrap', f: [{ t: 13, n: 'style-width', f: [{ t: 2, x: { r: ['~/tableWidth', '~/allowSelect'], s: "_1?'calc('+(_0-3)+'px - 2em)':(_0-3)+'px'" } }] }] }], f: [{ t: 16, r: '~/meta.extra', c: { r: '.' } }] });
        }

        // expando rows
        if (expandEl && expandEl.f && expandEl.f.length) {
          row[0].f[0].f.push({ t: 4, n: 50, x: { r: ['.', '~/expanded'], s: '_0===_1' }, f: [
            { t: 7, e: 'div', f: [{ t: 7, e: 'div', f: [{ t: 16, r: "~/meta.expand", c: { r: '.' } }] }], m: [{ t: 13, n: 'class-rtable-row-expand' }, { t: 4, n: 50, r: '~/noWrap', f: [{ t: 13, n: 'style-width', f: [{ t: 2, x: { r: ['~/tableWidth', '~/allowSelect'], s: "_1?'calc('+(_0-3)+'px - 2em)':(_0-3)+'px'" } }] }] }] }
          ] });
